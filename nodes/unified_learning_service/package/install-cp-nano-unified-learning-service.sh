...
NANO_SERVICE_NAME="Unified learning"
NANO_SERVICE_BIN_NAME="cp-nano-unified-learning-service"
NANO_SERVICE_INSTALLATION_FOLDER="unified_learning_service"

#Installable Names
CFG_FILE_NAME="cp-nano-unified-learning-service.cfg"
DBG_CONF_FILE_NAME="cp-nano-unified-learning-service-debug-conf.json"
SERVICE_CONF_FILE_NAME="service-conf.json"
NANO_SERVICE_BIN="unified_learning_service"
...
set_configuration()
{
    ...
    cp_exec "/etc/cp/scripts/cpnano_debug --default --service unified-learning-service"
...
run_installation()
{
    ...
    if [ -n "$cmd_pid_srv"  ]; then
        cp_print "Killing running instance(pid=$cmd_pid_srv) of the ${NANO_SERVICE_NAME} service on installation"
        kill -9 "$cmd_pid_srv"
    fi
