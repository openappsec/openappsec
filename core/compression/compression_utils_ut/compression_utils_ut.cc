...
// Verify decompressData overload with explicit CompressionType decompresses gzip correctly
TEST_F(CompressionUtilsTest, DecompressWithExplicitGzipType)
{
    Maybe<string> compressed = compressString(CompressionType::GZIP, simple_test_string);
    ASSERT_TRUE(compressed.ok());

    auto stream = initCompressionStream();
    unsigned char *data = reinterpret_cast<unsigned char *>(
        const_cast<char *>(compressed.unpack().c_str())
    );

    DecompressionResult result = decompressDataSafe(
        stream,
        CompressionType::GZIP,
        compressed.unpack().size(),
        data
    );
    finiCompressionStream(stream);

    ASSERT_EQ(result.ok, 1);
    string decompressed(reinterpret_cast<char *>(result.output), result.num_output_bytes);
    free(result.output);
    EXPECT_EQ(simple_test_string, decompressed);
}

// Verify decompressDataSafe with explicit BROTLI type decompresses Brotli data correctly
TEST_F(CompressionUtilsTest, DecompressWithExplicitBrotliType)
{
    Maybe<string> compressed = compressString(CompressionType::BROTLI, simple_test_string);
    ASSERT_TRUE(compressed.ok());
...
// Plain text with explicit BROTLI type must fail (no misclassification)
TEST_F(CompressionUtilsTest, DecompressPlainTextWithExplicitBrotliTypeFails)
{
    auto stream = initCompressionStream();
    unsigned char *data = reinterpret_cast<unsigned char *>(
        const_cast<char *>(simple_test_string.c_str())
    );

    DecompressionResult result = decompressDataSafe(
        stream,
        CompressionType::BROTLI,
        simple_test_string.size(),
        data
    );
    finiCompressionStream(stream);
