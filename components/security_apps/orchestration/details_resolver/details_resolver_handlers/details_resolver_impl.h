...
SHELL_CMD_HANDLER("QUID", "FS_PATH=<FILESYSTEM-PREFIX>;"
    ...
    "if [ \"${IS_MDS}\" = \"1\" ]; then "
        ...
        "/opt/CPotelcol/quid_api/get_mds_quid.json.${DOMAIN_NAME} 2>/dev/null | jq -r .message[0].QUID || echo '';"
    "else "
...
SHELL_CMD_HANDLER("ETH_MGMT_IP",
    ...
    "if [ \"${IS_MDS}\" = \"1\" ]; then "
    "DOMAIN_NAME=$(echo \"${FS_PATH}\" | grep -o -E \"domain-[^/]+\" | sed 's|domain-||'); "
    "[ -z \"${DOMAIN_NAME}\" ] && hostname -i || "
    "mgmt_cli -r true show-domain name \"${DOMAIN_NAME}\" -f json"
    " | jq -r '.servers[] | select(.active == true) | .\"ipv4-address\" // .\"ipv6-address\"'; "
    "else "
    "VS_ID=$(echo \"${FS_PATH}\" | grep -o -E \"vs[0-9]+\" | grep -o -E \"[0-9]+\"); "
    "ip_addr=''; "
    "if [ -z \"${VS_ID}\" ]; then "
    ...
    "ip_addr=$(ip -4 addr show \"${eth}\" 2>/dev/null | awk '/inet / {print $2}' | cut -d '/' -f1 | head -n 1); "
    "[ -z \"${ip_addr}\" ] && "
    "ip_addr=$(ip -6 addr show \"${eth}\" 2>/dev/null | awk '/inet6 / && !/fe80/ {print $2}' "
    "| cut -d '/' -f1 | head -n 1); "
    "else "
    "myobj=$(cat \"${FWDIR}/database/myown.C\" 2>/dev/null | grep ':name' | cut -d '(' -f 2 | tr -d ')'); "
    "ip_addr=$(sed -n \"/^ : (${myobj}$/,/^ : (/p\" "
    "\"${FWDIR}/state/local/FW1/local.vs_netobj\" 2>/dev/null"
    " | grep -m 1 ':ip_address ' | grep -oE '([0-9]+\\.){3}[0-9]+'); "
    "if [ -z \"${ip_addr}\" ]; then "
    "eth=\"$(grep \"management:instance:${VS_ID}:interface\" /config/active | awk '{print $2}')\"; "
    "ip_addr=$(ip -4 addr show \"${eth}\" 2>/dev/null | awk '/inet / {print $2}' | cut -d '/' -f1 | head -n 1); "
    "[ -z \"${ip_addr}\" ] && "
    "ip_addr=$(ip -6 addr show \"${eth}\" 2>/dev/null | awk '/inet6 / && !/fe80/ {print $2}' "
    "| cut -d '/' -f1 | head -n 1); "
