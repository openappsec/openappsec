...
public:
    ...
    struct Entry {
        std::string key;
        std::string sourceId;
        std::string asset_id;  // Asset identifier for routing to appropriate sync unit
        ...
        void serialize(Archive& ar) {
            ar(key, sourceId, asset_id, isTrusted, indicators, types);
        }

        // Serialize the entry to a binary buffer using cereal
        std::vector<char> serialize() const;
        
        // Deserialize the entry from a binary buffer using cereal
...
//     // Method to get serialized JSON data for REST
//     std::string getSerializedData() const {
//         std::ostringstream oss;
//         container->serialize(oss); // Uses your custom serialize(std::ostream&)
//         return oss.str();
//     }

// private:
//     std::shared_ptr<UnifiedIndicatorsContainer> container;
// };
