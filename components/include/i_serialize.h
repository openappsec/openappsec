...
// Template base classes to support different component types
template<typename ComponentType = WaapComponent>
class RestGetFileT : public ClientRest
{
public:
...
using RestGetFile = RestGetFileT<WaapComponent>;
...
template<typename ComponentType = WaapComponent>
class SerializeToFileBaseT :
    public I_Backup,
    public I_Serializable
{
public:
    SerializeToFileBaseT(const std::string &filePath);
    virtual ~SerializeToFileBaseT();
...
// Default specialization for backward compatibility
using SerializeToFileBase = SerializeToFileBaseT<WaapComponent>;

template<typename ComponentType = WaapComponent>
class SerializeToFilePeriodicallyT : public SerializeToFileBaseT<ComponentType>
{
public:
    SerializeToFilePeriodicallyT(std::chrono::seconds pollingIntervals, const std::string &filePath);
    virtual ~SerializeToFilePeriodicallyT();
...
using SerializeToFilePeriodically = SerializeToFilePeriodicallyT<WaapComponent>;

class WaapComponent;

template<typename ComponentType = WaapComponent>
class SerializeToLocalAndRemoteSyncBaseT : public I_RemoteSyncSerialize, public SerializeToFileBaseT<ComponentType>
{
public:
    SerializeToLocalAndRemoteSyncBaseT(std::chrono::minutes interval,
        std::chrono::seconds waitForSync,
        const std::string &filePath,
        const std::string &remotePath,
        const std::string &assetId,
        const std::string &owner);
    virtual ~SerializeToLocalAndRemoteSyncBaseT();
...
protected:
    ...
    {
        I_MainLoop *mainloop = Singleton::Consume<I_MainLoop>::by<ComponentType>();
...
protected:
    bool m_dataWasSent; // Flag to track if data was actually sent in the last postData() call
...
using SerializeToLocalAndRemoteSyncBase = SerializeToLocalAndRemoteSyncBaseT<WaapComponent>;
