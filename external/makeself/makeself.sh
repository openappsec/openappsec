...
MS_Usage()
{
    echo "Usage: $0 [params] archive_dir file_name label additional_args_help package_version startup_script [args]"
    ...
    echo "    --platform         : Check the platform's type before proceeding."
    echo
...
flag_exist=`grep "\-\-install" $archdir/$SCRIPT`
if [ -z "$flag_exist" ]; then
    echo "ERROR: Embedded script is missing --install property"
    exit 1
fi

flag_exist=`grep "\-\-uninstall" $archdir/$SCRIPT`
if [ -z "$flag_exist" ]; then
    echo "ERROR: Embedded script is missing --uninstall property"
    exit 1
fi

flag_exist=`grep "\-\-pre_install_test" $archdir/$SCRIPT`
if [ -z "$flag_exist" ]; then
    echo "ERROR: Embedded script is missing --pre_install_test handler property"
    exit 1
fi

flag_exist=`grep "\-\-post_install_test" $archdir/$SCRIPT`
if [ -z "$flag_exist" ]; then
    echo "ERROR: Embedded script is missing --post_install_test handler property"
    exit 1
fi
